...
public class HelperToolsTests {
	...
	public void checkCmdHereStillExists() throws IOException{	
		startUpAndIncreaseTimeOutFor("http://www.microsoft.com");

		selenium.open("http://www.microsoft.com/windowsxp/Downloads/powertoys/Xppowertoys.mspx");
		// find the cmdHere.exe
		assertEquals(1,selenium.getXpathCount("//a[.='CmdHere.exe']").intValue());
	...
	public void checkConsoleToolStillExists() throws IOException{
		...
		assertEquals("Console | Get Console at SourceForge.net", selenium.getTitle());		
	}	
	
	@Test
	/**
	 * This test checks that the StExBar tool still exists
	 * If this test fails then we need to update:
	 * - Step Two - Run Selenium-RC 
	 * and the relevant appendix
	 */
	public void checkStExBarToolStillExists() throws IOException{
		startUpAndIncreaseTimeOutFor("http://tools.tortoisesvn.net");
		
		selenium.open("http://tools.tortoisesvn.net/StExBar");
		assertEquals("StExBar | Stefan's Tools", selenium.getTitle());		
	}	
	...
	public void checkSeleniumIDEStillExists() throws IOException{
		...
		assertEquals(1,selenium.getXpathCount("//a[@href='http://release.seleniumhq.org/selenium-ide/1.0.6/selenium-ide-1.0.6.xpi']").intValue());
	}		
	...
	public void checkEclipseIDEStillExistsWithDownloads() throws IOException{
		...
		assertEquals("Eclipse downloads - mirror selection", selenium.getTitle());
	}	
	...
	public void checkJUnitORgStillExists() throws IOException{
		...
		// installing Ant
		assertTrue(selenium.isElementPresent("css=a[href='http://sourceforge.net/project/showfiles.php?group_id=15278&package_id=12472']"));
		assertEquals("Download JUnit",selenium.getText("css=a[href='http://sourceforge.net/project/showfiles.php?group_id=15278&package_id=12472']"));
	...
	@Test
	public void checkJavaProgrammingTutorials() throws IOException{
		startUpAndIncreaseTimeOutFor("http://www.programmingtutorials.com");
		
		selenium.open("http://www.programmingtutorials.com/java.aspx");
		assertTrue(selenium.isElementPresent("xpath=//span[@id='Tutorial1_TutorialsLabel' and .='Java Tutorials']"));
	}	
	...
	public void check_Ant() throws IOException{
		...
		// check version incase update causes issues with scripts
		// if this fails then update to new ant and check builds still work
		// then update this test
		assertTrue(selenium.isTextPresent("Currently, Apache Ant 1.8.0 is the best available version")); 
	...
	public void check_JavaJDK() throws IOException{
		...
		selenium.open("http://java.sun.com/javase/downloads/index.jsp");
		assertEquals("Java SE Downloads - Sun Developer Network (SDN)", selenium.getTitle());
		assertTrue("Java no longer version JDK 6", selenium.isElementPresent("css=a[title^='Download Java SE Development Kit (JDK) 6 Update']"));
